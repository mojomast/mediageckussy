...
export function registerProjectRoutes(app: Express) {
  ...
  app.post("/api/projects", async (req, res) => {
    const { canonYaml, outputDir } = req.body as { canonYaml?: string; outputDir?: string };
    if (!canonYaml || !outputDir) {
      return fail(res, new Error("canonYaml and outputDir are required"), 400);
    }

    const absoluteOutputDir = path.resolve(outputDir);
    const canonPath = path.join(absoluteOutputDir, "00_admin/canon_lock.yaml");
    await fs.ensureDir(path.dirname(canonPath));
    await fs.writeFile(canonPath, canonYaml, "utf8");

    return streamJob(res, async (send) => {
      send("started", { step: "generate" });
      const result = await generatePackage({ canonPath, outputDir: absoluteOutputDir });
  ...
  app.get(/^\/api\/projects\/([^/]+)\/files\/(.+)$/, async (req, res) => {
    try {
      ...
      const absolutePath = path.join(projectDir(slug), relativePath);
